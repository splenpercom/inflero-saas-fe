...
import { fetchBrands, createBrand, updateBrand, deleteBrand } from "../../api/inventory";
...
export function Brands() {
  ...
  const loadBrands = useCallback(async () => {
    ...
    try {
      ...
      notifyFromError(err, tr("Brendləri yükləmək alınmadı", "Failed to load brands"));
    } finally {
      setLoading(false);
    }
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text(tr("Brendlər Hesabatı", "Brands Report"), 14, 20);
    ...
    doc.save(`${tr("brendler", "brands")}-${new Date().getTime()}.pdf`);
  };
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `${tr("brendler", "brands")}-${new Date().getTime()}.csv`);
  ...
  const handleRefresh = () => {
    ...
    void loadBrands().finally(() => setIsRefreshing(false));
  };
  ...
  const handleDownloadDemo = () => {
    ...
    const demoData = [
      [tr("Demo Brend 1", "Demo Brand 1"), "11 Mar 2026", tr("Aktiv", "Active")],
      [tr("Demo Brend 2", "Demo Brand 2"), "10 Mar 2026", tr("Aktiv", "Active")],
      [tr("Demo Brend 3", "Demo Brand 3"), "09 Mar 2026", tr("Qeyri-aktiv", "Inactive")],
    ];
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute("href", url);
    link.setAttribute("download", `${tr("brendler-demo", "brands-demo")}.csv`);
    link.style.visibility = "hidden";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
        ...
        setImportProgress((prev) => {
          if (prev >= 100) {
            ...
            setTimeout(() => {
              ...
              alert(tr("Brendlər uğurla idxal edildi!", "Brands imported successfully!"));
            }, 500);
            return 100;
          }
          return prev + 10;
  ...
  const handleDelete = async (id: string) => {
    if (!(await askConfirm({
      ...
      message: tr("Bu brendi silmək istədiyinizə əminsiniz?", "Are you sure you want to delete this brand?"),
      variant: "danger",
    }))) return;
    ...
    try {
      ...
      notifySuccess(tr("Brend silindi", "Brand deleted"));
    } catch (err) {
      notifyFromError(err);
    }
  };
  ...
  const handleSubmitBrand = async () => {
    ...
    if (!name) {
      errors.name = tr("Brend adı tələb olunur", "Brand name is required");
    } else {
      const duplicate = brands.some(
        (b) => b.name.trim().toLowerCase() === name.toLowerCase() && b.id !== editingId,
      );
      if (duplicate) {
        errors.name = tr("Bu adda brend artıq mövcuddur", "A brand with this name already exists");
      }
    }
    ...
    try {
      if (isEdit) {
        ...
        notifySuccess(tr("Brend uğurla yeniləndi!", "Brand updated successfully!"));
      } else {
        const created = await createBrand({ name: formData.name.trim(), status: formData.status });
        setBrands((prev) => [
          ...prev,
          {
            id: created.id,
      ...
      <div className="p-4 sm:p-4 xl:p-6 2xl:px-8 py-4">
        ...
        <div className="mb-4">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {tr("Brendlərinizi idarə edin", "Manage your brands")}
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-3 mb-4">
          <div className="flex flex-col lg:flex-row gap-3 items-stretch lg:items-center">
            ...
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder={tr("Brend axtar...", "Search brands...")}
                value={searchQuery}
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                <span>{tr("Brend Əlavə Et", "Add Brand")}</span>
              </button>
              )}
            </div>
          </div>
                  ...
                  >
                    <td className="px-3 py-2 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-lg bg-[#e8ebff] dark:bg-[#0026f6]/20 flex items-center justify-center text-xs font-bold text-[#0026f6] dark:text-[#0026f6]">{brand.name.charAt(0).toUpperCase()}</div>
                    ...
                    <td className="px-3 py-2 whitespace-nowrap">
                      ...
                      >
                        {brand.status === "active" ? tr("Aktiv", "Active") : tr("Qeyri-aktiv", "Inactive")}
                      </span>
                    </td>
                    <td className="px-3 py-2 whitespace-nowrap">
                      <div className="flex items-center gap-2">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-md border border-gray-200 dark:border-gray-800">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                  {tr("Brendləri İdxal Et", "Import Brands")}
                </h2>
                <button
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow-2xl w-full max-w-sm border border-gray-200 dark:border-gray-800">
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-800">
                <h2 className="text-sm font-semibold text-gray-900 dark:text-white">
                  {editingId ? tr("Brendi Redaktə Et", "Edit Brand") : tr("Brend Əlavə Et", "Add Brand")}
                </h2>
                <button
              ...
              <div className="p-4 space-y-3">
                <div>
                  <label className="text-xs font-medium text-gray-900 dark:text-white mb-1.5 block">
                    {tr("Brend Adı *", "Brand Name *")}
                  </label>
                  <input
                    type="text"
                    placeholder={tr("Brend adını daxil edin", "Enter brand name")}
                    value={formData.name}
