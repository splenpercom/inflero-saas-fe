...
export function AddBillerModal({ isOpen, onClose, onSave }: AddBillerModalProps) {
  ...
  const generateCode = () => {
    const randomCode = "BL" + Math.floor(Math.random() * 10000).toString().padStart(3, "0");
    setCode(randomCode);
  };

  const handleSave = () => {
    if (code && name && email && phone && company && status) {
      onSave({
        code,
        name,
        email,
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-800 sticky top-0 bg-white dark:bg-gray-900 z-10">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-white">
            {tr("Ödəyici Əlavə Et", "Add Biller")}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="text-xs font-medium text-gray-900 dark:text-white mb-1.5 block">
                {tr("Şirkət", "Company")} <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                value={company}
