...
interface ManageIncomeCategoriesModalProps {
  isOpen: boolean;
  onClose: () => void;
  onChanged?: () => void;
}

export function ManageIncomeCategoriesModal({
  isOpen,
  onClose,
  onChanged,
}: ManageIncomeCategoriesModalProps) {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      if (isEditMode && editingId) {
        await updateIncomeCategory(editingId, {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-2xl w-full max-w-2xl border border-gray-200 dark:border-gray-800 max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-800">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-white">{tr("Gəlir Kateqoriyalarını İdarə Et", "Manage Income Categories")}</h2>
          ...
          {isAddCategoryOpen && (
            <div className="mb-4 p-3 border border-gray-200 dark:border-gray-800 rounded-lg bg-gray-50 dark:bg-gray-800/30">
              <h3 className="text-xs font-semibold text-gray-900 dark:text-white mb-3">
                {isEditMode ? tr("Kateqoriyanı Redaktə Et", "Edit Category") : tr("Gəlir Kateqoriyası Əlavə Et", "Add Income Category")}
              </h3>
