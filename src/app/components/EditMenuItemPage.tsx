...
import { Info, DollarSign, Upload, X, ArrowLeft, Leaf, Egg, Fish, Coffee, Plus, Trash2, ChevronDown, Check } from "lucide-react";
...
export function EditMenuItemPage() {
    ...
    if (itemId) {
      ...
      if (item) {
        setFormData({
          ...
          itemType: item.isVeg ? "veg" : "non-veg",
  ...
  }, [itemId, getMenuItemById, navigate]);
  ...
  const updateVariation = (variationId: string, field: "name" | "dineInPrice" | "pickupPrice" | "deliveryPrice" | "dineInEnabled" | "pickupEnabled" | "deliveryEnabled", value: string | boolean) => {
    setVariations(
                ...
                <div>
                  ...
                  >
                    <button
                      ...
                      className="w-full flex items-center justify-between px-3 py-1.5 text-xs border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white hover:border-[#0026f6] focus:outline-none focus:ring-2 focus:ring-[#0026f6] focus:border-[#0026f6] transition-all"
                    >
                      <span>{formData.isAvailable ? t.yes : t.no}</span>
                      <ChevronDown className={`w-3.5 h-3.5 text-gray-400 transition-transform duration-200 ${isAvailableDropdownOpen ? "rotate-180" : ""}`} />
                    </button>
